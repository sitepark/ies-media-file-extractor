...
class ContentExtractorHandlerTest {
	...
	void testWrite() throws SAXException {
		ContentExtractorHandler handler = new ContentExtractorHandler(
				new StringWriter(),
				100);
	...
	void testToTinyString() throws SAXException {
		...
		assertEquals(
				"abcdefghijk abc cde",
				handler.toTidyString(),
				"content should be cleaned of whitespaces");
	}
	...
	void testWriteUnlimited() throws SAXException {
		...
		assertEquals(
				iterations * s.length(),
				handler.toString().length(),
				"content should not be trancated");
	}
}
